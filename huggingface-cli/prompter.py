...
Description: CLI engine for standardized HuggingFace queries via BEJSON schemas.
    ...
    for row in data["Values"]:
        if search_id:
            if row[m_id_idx] == search_id:
                return {"id": row[m_id_idx]}
        elif row[active_idx] is True:
            return {"id": row[m_id_idx]}
    
    if search_id:
        return {"id": search_id}
    return {"id": "meta-llama/Llama-3-8B-Instruct"} # Fallback
...
def get_keys(key_path):
    ...
    if k_idx == -1: return []
    return [row[k_idx] for row in data["Values"] if "YOUR_HUGGINGFACE_KEY" not in str(row[k_idx])]
...
def send_query(prompt, router, model_override=None, output_file=None):
    ...
    while attempts < len(keys):
        ...
        # Standard HuggingFace Chat Completion style prompt
        payload = {
        ...
        try:
            ...
            # HuggingFace typically returns a list of results
            if isinstance(data, list) and len(data) > 0:
                ...
                raise Exception("No generated text returned")
...
def main():
    parser = argparse.ArgumentParser(description="Hugging Face Standardized Prompter")
