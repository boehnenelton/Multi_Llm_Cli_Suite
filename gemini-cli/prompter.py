#!/usr/bin/env python3
"""
Orchestrator: prompter.py
Description: CLI engine for standardized Gemini queries via BEJSON schemas.
...
def get_active_model(model_path, search_id=None):
    ...
    if search_id:
        return {"id": search_id, "thinking": False, "search": False}
    return {"id": "gemini-2.5-flash", "thinking": False, "search": False} # Fallback
...
def get_keys(key_path):
    ...
    if k_idx == -1: return []
    return [row[k_idx] for row in data["Values"] if "YOUR_GEMINI_KEY" not in str(row[k_idx])]
...
def send_query(prompt, router, model_override=None, output_file=None):
    ...
    if model_cfg["search"]: print("[*] Feature: Google Search ENABLED")
    ...
    while attempts < len(keys):
        ...
        try:
            ...
            # Security: Mask key in error message if it appears in the URL
            err_msg = str(e)
            if key in err_msg:
...
def main():
    parser = argparse.ArgumentParser(description="Gemini Standardized Prompter")
    parser.add_argument("prompt", nargs="?", help="The prompt string")
    parser.add_argument("--model", help="Override the active model ID (e.g., gemini-2.0-flash-exp)")
