...
Description: CLI engine for standardized Groq queries via BEJSON schemas.
...
def get_active_model(model_path, search_id=None):
    ...
    return {"id": "llama-3.3-70b-versatile", "thinking": False, "search": False} # Fallback
...
def get_keys(key_path):
    ...
    if k_idx == -1: return []
    return [row[k_idx] for row in data["Values"] if "YOUR_GROQ_KEY" not in str(row[k_idx]) and "KEY_HERE" not in str(row[k_idx])]
